...
% Nonlinear terms
[Fr_handle,BeD_handle] = make_handle(conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP);
...
for k=1:length(time)
    ...
    Fun = @(zm) [  p(:,k) + (dt/2)*(-iMpr'*Fr_handle(zm(N_free+M_Omega+1:end),rD(:,k))'*iMe'*Ke*zm(N_free+1:N_free+M_Omega) + iMpr'*br) - zm(1:N_free);
                 eps(:,k) + (dt/2)*(iMe*Fr_handle(zm(N_free+M_Omega+1:end),rD(:,k))*iMpr*Mpp*zm(1:N_free) + BeD_handle(zm(N_free+M_Omega+1:end),rD(:,k))*Ind_D*vD(tm)) - zm(N_free+1:N_free+M_Omega);
                  rr(:,k) + (dt/2)*(iMpr*Mpp*zm(1:N_free) - BvD*vD(tm)) - zm(N_free+M_Omega+1:end)];
...
function [Fr,BeD] = NL_terms_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP)
    ...
    Fr   = F_NL(:,dof_F);
    BeD  = iMe*(F_NL(:,dof_D) - Fr*iMpr*MpD);

end

function Fr = Fr_handle_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP)
     [Fr,~] = NL_terms_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP);
end

function BeD = BeD_handle_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP)
     [~,BeD] = NL_terms_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP);
end

function [Fr_handle,BeD_handle] = make_handle(conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP)
    Fr_handle  = @(rr,rD)  Fr_handle_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP);
    BeD_handle = @(rr,rD) BeD_handle_fun(rr,rD,conec,NN,NE,Le,prop,dof_F,dof_D,iMe,iMpr,MpD,MP);
end