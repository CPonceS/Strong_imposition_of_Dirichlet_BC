...
prop.Didx = [1 0 1];           % Logical index relative to which generalized velocities are applied as control inputs.
prop.Nidx = [0 0 0];           % Logical index relative to which generalized tractions are applied as control inputs.

% Initial configuration
prop.r0  = @(s) s.*[0;-1;0];
prop.dr0 = [0;-1;0];           % ||dr0|| = 1 
...
% Nonlinear terms
[Fer_handle,eps_handle] = make_handle(conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
...
% decrease dt if vD is more "aggressive"
Amp  = 0.1; 
freq = 2*pi*2;
vD = @(t) [-freq*Amp*sin(freq*t);freq*Amp*cos(freq*t)].*(t<0.5);
...
for k=1:length(time)
    ...
    Fun = @(zm) [ p(:,k) + (dt/2)*(-iMpr'*(Fer_handle(zm(N_free+1:end),rD(:,k)) - br)) - zm(1:N_free);
                 rr(:,k) + (dt/2)*(iMpr*Mpp*zm(1:N_free) - BvD*vD(tm)) - zm(N_free+1:end)];
    
    zm = fsolve(Fun,z0,opciones_fsolve); z0 = zm; 
    z  = 2*zm - z;    

    p(:,k+1)  = z(1:N_free);
    rr(:,k+1) = z(N_free+1:end);
    rD(:,k+1) = rD(:,k) + dt*Ind_D*vD(tm); 

    E_kin(k) = 0.5*p(:,k)'*Mpp*p(:,k);
    E_ela(k) = 0.5*eps_handle(rr(:,k),rD(:,k))'*Ke*eps_handle(rr(:,k),rD(:,k));
    E_pot(k) = Epot_ref - (vr0+rr(:,k))'*br - (vrD0+rD(:,k))'*bD;
...
title(strcat({'N$^\circ$ of elements: '}, num2str(NE), {', Shape functions: P1'}),'Interpreter','latex','FontSize',14)
hold on, box on
...
if isequal(anim,1)
    ...
    for i=1:5:k
        ...
        plot3(x_actual(1,1:i),z_actual(1,1:i),y_actual(1,1:i),'g.')
        plot3(x_actual(1,i),z_actual(1,i),y_actual(1,i),'r.','MarkerSize',20)
        text(0.45,0.8,-0.1,strcat({'Time = '},num2str(round(time(i),1)),{' [s]'}),'Interpreter','latex','FontSize',12)
    
        xlim([-1 1]*L*0.51)    , xl = xlim;
        ylim([-1 1]*L*0.51)    , yl = ylim;
        zlim([-1 0.0]*L*1.1)   , zl = zlim;
...
function [Fer,eps_hat] = NL_terms_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
    ...
    epsilon = (prop.dr0(1)*dr1 + prop.dr0(2)*dr2 + prop.dr0(3)*dr3) + 0.5*(dr1.^2 + dr2.^2 + dr3.^2);
    
    FNp_local = [-F1NL_1 F1NL_1 -F1NL_2 F1NL_2 -F1NL_3 F1NL_3]; 
    Fer_local = FNp_local*E*A0.*epsilon; 
    idx_total = [conec conec+NN conec+2*NN];
    
    Fer_NL  = sparse(idx_total(:),1,Fer_local(:),3*NN,1);
    Fer     = Fer_NL(dof_F);
    eps_hat = iMe*epsilon.*Le;

end
 
function Fer = Fr_handle_fun(rDom,rDir,conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
     [Fer,~] = NL_terms_fun(rDom,rDir,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
end

function eps_hat = eps_handle_fun(rDom,rDir,conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
     [~,eps_hat] = NL_terms_fun(rDom,rDir,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
end

function [Fr_handle,eps_handle] = make_handle(conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
    Fr_handle  = @(rr,rD)  Fr_handle_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
    eps_handle = @(rr,rD) eps_handle_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
end
