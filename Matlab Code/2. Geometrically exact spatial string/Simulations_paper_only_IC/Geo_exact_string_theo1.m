...
for k=1:length(time)
    ...
    Fun = @(zm) [ p(:,k) + (dt/2)*(-iMpr'*(Fer_handle(zm(N_free+1:end),rD(:,k)) - br)) - zm(1:N_free);
                 rr(:,k) + (dt/2)*(iMpr*Mpp*zm(1:N_free)) - zm(N_free+1:end)];
...
function Fer = Fr_handle_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
     [Fer,~] = NL_terms_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
end

function eps_hat = eps_handle_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0)
     [~,eps_hat] = NL_terms_fun(rr,rD,conec,NN,iMe,Le,prop,dof_F,MP,E,A0);
end
