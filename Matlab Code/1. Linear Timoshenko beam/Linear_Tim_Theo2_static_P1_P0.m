...
for p = 1:N_gauss
    ...
    Id_local = Id_local + W_g(p)*((Le/2).*N_xi); 
    D_local  = D_local  + W_g(p)*dN_dxi;        
end
% Assembly:
...
Id_glob = sparse(I_vec(:),J_mv(:),Id_local(:),NN,NE); % discretization of operator 1    (mixed)
D_glob  = sparse(I_vec(:),J_mv(:),D_local(:),NN,NE);  % discretization of operator d/dx (mixed)

%% Matrices of the model
I_nPDE = eye(n_PDE);
I_mPDE = eye(m_PDE);

Fx = kron(prop.F0,Id_glob') + kron(prop.F1,D_glob'); 
Mn = kron(I_nPDE,M_glob);
b  = kron(prop.b,V_glob);
  
iMe = kron(I_mPDE,sparse(diag(1./Le)));
Ke  = kron(prop.K,sparse(diag(Le))); 
...
Krr = iMpr'*Fr'*iMe'*Ke*iMe*Fr;


%% Static solution (Theorem 1, P1-P0)
