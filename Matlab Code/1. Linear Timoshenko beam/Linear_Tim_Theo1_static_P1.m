...
% Integration:
% Full:     Exact for P1  -> Causes shear locking
% Reduced:  1-point Gauss -> Prevet shear locking
Integration = 'Reduced'; % 'Full' or 'Reduced'

k_bend    = (E*I0./Le).*[1 -1 -1 1];              % Bending: Integral of (psi')^2                  
k_sh_dw   = (kappa*G*A0./Le).*[1 -1 -1 1];        % Shearing: Integral of (w')^2
k_sh_psiw = (kappa*G*A0*ones(NE,1)).*[1/2  1/2 -1/2 -1/2]; % rows: psi, cols: w
k_sh_wpsi = (kappa*G*A0*ones(NE,1)).*[1/2 -1/2  1/2 -1/2]; % rows: w, cols: psi

if strcmp(Integration,'Full') 
    k_sh_psi = (kappa*G*A0.*Le).*[1/3 1/6 1/6 1/3]; 
elseif strcmp(Integration,'Reduced')
    k_sh_psi = (kappa*G*A0.*Le).*[1/4 1/4 1/4 1/4]; 
end
...
text(0.29,0.37,strcat(Integration,{' integration'}),'Interpreter','latex','FontSize',14,'HorizontalAlignment','center','VerticalAlignment','middle')
grid on
