...
%% Discretization (Theorem 2, P2-P2)
...
for p = 1:N_gauss
    ...
    D_local  = D_local + W_g(p)*repmat(D_mat(:)',NE,1);
end
% Assembly:
...
D_glob = sparse(I_mat(:),J_mat(:),D_local(:),NN,NN);

%% Matrices of the model
I_nPDE = eye(n_PDE);
I_mPDE = eye(m_PDE);

Fx  = kron(prop.F0,M_glob) + kron(prop.F1,D_glob);
Mn  = kron(I_nPDE,M_glob);
b   = kron(prop.b,V_glob);

Ke  = kron(prop.K,M_glob);
Me  = kron(I_mPDE,M_glob);   iMe = sparse(eye(M_Omega)/Me);
...
%% Static solution (Theorem 2, P2-P2)
...
title(strcat({'N$^\circ$ of elements: '}, num2str(NE), {', Shape functions: P2-P2'}),'Interpreter','latex','FontSize',14)
for i=1:length(factor)  
